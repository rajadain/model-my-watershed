...
export PGHOST="${MMW_DB_HOST:-postgres}"
export PGDATABASE="${MMW_DB_NAME:-mmw}"
export PGUSER="${MMW_DB_USER:-mmw}"
export PGPASSWORD="${MMW_DB_PASSWORD:-mmw}"
PUBLIC_HOSTED_ZONE_NAME="${MMW_PUBLIC_HOSTED_ZONE_NAME:-}"
...
function purge_tile_cache {
    if [ -z "$PUBLIC_HOSTED_ZONE_NAME" ] ; then
        echo "MMW_PUBLIC_HOSTED_ZONE_NAME not set; skipping cache purge"
        return
    fi
...
function create_trgm_indexes {
    ...
    for table in "${TRGM_TABLES[@]}"; do
        psql -c "CREATE INDEX IF NOT EXISTS trgm_idx_${table}_name ON ${table} USING gin (name gin_trgm_ops);"
    done
}
...
if [ "$load_mapshed" = "true" ] ; then
    ...
    FILES=("ms_weather.sql.gz" "ms_weather_station_1523624744.sql.gz"
           ...
           "ms_pointsource_pa.sql.gz" "ms_county_animals.sql.gz")
